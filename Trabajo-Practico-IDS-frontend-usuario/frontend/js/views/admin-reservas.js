            ...
            ${localReservas.map(res => {
              ...
              return `
                <tr>
                  ...
                  <td>${user ? user.nombre : `Usuario #${res.id_usuario}`}</td>
                 
                  <td>${sala ? sala.salon_nombre : `Sala #${res.id_salon}`}</td>
                  ...
                  <td>
                    ...
                    <span class="badge" style="padding:4px 6px; border-radius:4px; color:#fff; background: ${
                      res.estado === 'aprobada' ? 'var(--color-accent-green)' : res.estado === 'pendiente' ? 'var(--color-accent-orange)' : 'var(--color-accent-red)'
  ...
  function abrirFormularioReserva(esEdicion = false, id = null) {
    ...
    const camposHTML = `
      ...
      <div class="form-group">
        ...
        <select name="estado" class="form-control">
          ...
          <option value="aprobada" ${resExistente.estado === 'aprobada' ? 'selected' : ''}>Aprobada / Confirmada</option>
    ...
    abrirModal(
      ...
      async (datosFormulario) => {
        ...
        if (esEdicion) {
          // Despacha una solicitud de actualización parcial por método PUT inyectando el ID correspondiente
          ...
          // Despacha una solicitud de creación por método POST al endpoint raíz de reservas
