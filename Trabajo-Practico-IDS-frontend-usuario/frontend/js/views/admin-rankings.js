...
export async function renderRankingAulas(container) { 
  // En lugar de pegarle al puerto 3000, consumimos el motor persistente local
  const salonesBase = await window.StudyRoomAPI.getSalones() || [];
  
  // Mapeamos y extendemos los salones simulando las métricas de estrellas y comentarios
  localRankingSalones = salonesBase.map((s, index) => ({
    id: s.id,
    salon_nombre: s.salon_nombre,
    // Asigna valores consistentes de prueba basados en el ID o posición del elemento
    estrellas: s.id === 1 ? 4.8 : s.id === 2 ? 4.2 : 3.9,
    comentarios: s.id === 1 ? 24 : s.id === 2 ? 18 : 5
  }));
...
export async function renderRankingUsuarios(container) { 
  // Forzamos directamente la lectura del almacenamiento local simulado eliminando la llamada conflictiva a red
  localRankingUsers = await window.StudyRoomAPI.getUsuarios(); 
