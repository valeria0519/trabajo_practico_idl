...
export async function renderObjetos(container) {
  ...
  function abrirFormularioObjeto(esEdicion = false, id = null) {
    ...
    abrirModal(
      ...
      async (datosFormulario) => {
        ...
        if (esEdicion) {
          await window.StudyRoomAPI.editarObjeto(objetoExistente.id, {
            id_salon: idDestino,
            funciona: funcionaBoolean
          });
        } else {
          await window.StudyRoomAPI.crearObjeto({
            objeto: datosFormulario.objeto,
            id_salon: idDestino,
            funciona: funcionaBoolean
          });
        }
  ...
  async function eliminarObjeto(id) {
    ...
    if (confirm(`¿Desea eliminar permanentemente el objeto #${id} del inventario de hardware?`)) {
      await window.StudyRoomAPI.eliminarObjeto(id);
